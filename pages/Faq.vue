<template>
  <div class="faq">
    ...
    <div class="middle-panel">
      ...
      <div class="list">
        <div v-for="(faq, index) in faqData" :key="index" class="item">
          <div class="caption" @click="openContent(index)">
            <font-awesome-icon
              icon="fa-solid fa-plus"
              :class="openFaq === index ? 'opened' : ''"
            ></font-awesome-icon>
            <h1>{{ index }}</h1>
          </div>
          <div class="faqarea" :class="openFaq === index ? 'active' : ''">
            <div v-for="(content, i) in faq" :key="i" class="listfaq">
              <div class="headfaq">
...
export default {
  ...
  async asyncData({ $axios, req, store }) {
    ...
    await store.dispatch("default/fetchFaq");
    const response = store.getters["default/getFaqs"];
